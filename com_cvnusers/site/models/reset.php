...
class UsersModelReset extends JModelForm
{
	...
	{
		// Get the form.
		$form = $this->loadForm('com_cvnusers.reset_request', 'reset_request', array('control' => 'jform', 'load_data' => $loadData));
		if (empty($form)) {
			return false;
		}
	...
	{
		// Get the form.
		$form = $this->loadForm('com_cvnusers.reset_complete', 'reset_complete', $options = array('control' => 'jform'));
		if (empty($form)) {
			return false;
		}
	...
	{
		// Get the form.
		$form = $this->loadForm('com_cvnusers.reset_confirm', 'reset_confirm', $options = array('control' => 'jform'));
		if (empty($form)) {
			return false;
		}
	...
	function processResetComplete($data)
	{
		...
		$token	= $app->getUserState('com_cvnusers.reset.token', null);
		$userId	= $app->getUserState('com_cvnusers.reset.user', null);
		...
		$app->setUserState('com_cvnusers.reset.token', null);
		$app->setUserState('com_cvnusers.reset.user', null);
	...
	function processResetConfirm($data)
	{
		...
		$app = JFactory::getApplication();
		$app->setUserState('com_cvnusers.reset.token', $crypt.':'.$salt);
		$app->setUserState('com_cvnusers.reset.user', $user->id);
	...
	public function processResetRequest($data)
	{
		...
		$link = 'index.php?option=com_cvnusers&view=reset&layout=confirm'.$itemid;
