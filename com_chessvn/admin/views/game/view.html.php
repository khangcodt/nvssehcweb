 ...
 * @subpackage 	Chessvn
 */
class ChessvnViewGame extends JViewLegacy
{
	/**
	...
	function display($tpl = null){
		...
		$isNew = ($this->item->gameid == null);

		$text = $isNew ? JText::_( 'JACTION_CREATE' ) : JText::_( 'JACTION_EDIT' );
		JToolBarHelper::title(   JText::_( 'COM_CHESSVN_MENU_GAMELIST' ).': <span>[ ' . $text.' ]</span>' );
		
		if ($isNew)  {
			if($user->authorise('core.create', 'com_chessvn')){
				JToolBarHelper::apply('game.apply');
				JToolBarHelper::save('game.save');
			}
			JToolBarHelper::cancel('game.cancel');
		} else {
		...
		$dataOptions = array();
		foreach(explode(',', 'status,completionstatus,nextmove,drawrequest') as $field){
			if (!$field) continue;
