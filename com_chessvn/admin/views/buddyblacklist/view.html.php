 ...
 * @subpackage 	Chessvn
 */
class ChessvnViewBuddyblacklist extends JViewLegacy
{
	/**
	...
	function display($tpl = null){
		...
		$isNew = ($this->item->bblid == null);

		$text = $isNew ? JText::_( 'JACTION_CREATE' ) : JText::_( 'JACTION_EDIT' );
		JToolBarHelper::title(   JText::_( 'COM_CHESSVN_MENU_BUDDYBLACKLISTLIST' ).': <span>[ ' . $text.' ]</span>' );
		
		if ($isNew)  {
			if($user->authorise('core.create', 'com_chessvn')){
				JToolBarHelper::apply('buddyblacklist.apply');
				JToolBarHelper::save('buddyblacklist.save');
			}
			JToolBarHelper::cancel('buddyblacklist.cancel');
		} else {
			...
			JToolBarHelper::cancel( 'buddyblacklist.cancel', 'JTOOLBAR_CLOSE' );
		}
