 ...
 * @subpackage 	Chessvn
 */
class ChessvnViewMovehistorylist extends JViewLegacy
{
	/**
	 * Movehistorylist view display method
	 * @return void
	 **/
	function display($tpl = null){
		...
		JToolBarHelper::title( JText::_( 'COM_CHESSVN_MENU_MOVEHISTORYLIST' ), 'generic.png' );
		if($this->user->authorise('core.edit', 'com_chessvn')) JToolBarHelper::editList('movehistory.edit');
		if($this->user->authorise('core.create', 'com_chessvn')) JToolBarHelper::addNew('movehistory.add');
		if($this->user->authorise('core.delete', 'com_chessvn')) JToolBarHelper::deleteList('', 'movehistorylist.delete');
		
		if( (isset($this->rows[0]->published)) && ($this->user->authorise('core.edit', 'com_chessvn')) ){
			JToolBarHelper::divider();
			JToolBarHelper::publish('movehistorylist.publish');
			JToolBarHelper::unpublish('movehistorylist.unpublish');
		}
	...
	protected function auth($area){
		//echo '<pre>' . print_r(JAccess::getActions('com_chessvn','movehistory'),true).'</pre>';die();
		$aclLocal = array();
		foreach(JAccess::getActions('com_chessvn', 'movehistory') as $ar) $aclLocal[] = $ar->name;
		if(in_array($area, $aclLocal)){
			return $this->user->authorise($area, 'com_chessvn', 'movehistory');
		}else{
			$aclGlobal = array();
