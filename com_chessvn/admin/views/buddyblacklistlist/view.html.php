...
{
	/**
	 * Buddyblacklistlist view display method
	 * @return void
	 **/
	function display($tpl = null){
		...
		JToolBarHelper::title( JText::_( 'COM_CHESSVN_MENU_BUDDYBLACKLISTLIST' ), 'generic.png' );
		if($this->user->authorise('core.edit', 'com_chessvn')) JToolBarHelper::editList('buddyblacklist.edit');
		if($this->user->authorise('core.create', 'com_chessvn')) JToolBarHelper::addNew('buddyblacklist.add');
		if($this->user->authorise('core.delete', 'com_chessvn')) JToolBarHelper::deleteList('', 'buddyblacklistlist.delete');
		
		if( (isset($this->rows[0]->published)) && ($this->user->authorise('core.edit', 'com_chessvn')) ){
			JToolBarHelper::divider();
			JToolBarHelper::publish('buddyblacklistlist.publish');
			JToolBarHelper::unpublish('buddyblacklistlist.unpublish');
		}
	...
	protected function auth($area){
		//echo '<pre>' . print_r(JAccess::getActions('com_chessvn','buddyblacklist'),true).'</pre>';die();
		$aclLocal = array();
		foreach(JAccess::getActions('com_chessvn', 'buddyblacklist') as $ar) $aclLocal[] = $ar->name;
		if(in_array($area, $aclLocal)){
			return $this->user->authorise($area, 'com_chessvn', 'buddyblacklist');
		}else{
			$aclGlobal = array();
