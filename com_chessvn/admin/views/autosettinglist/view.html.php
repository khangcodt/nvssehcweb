 ...
 * @subpackage 	Chessvn
 */
class ChessvnViewAutosettinglist extends JViewLegacy
{
	/**
	 * Autosettinglist view display method
	 * @return void
	 **/
	function display($tpl = null){
		...
		JToolBarHelper::title( JText::_( 'COM_CHESSVN_MENU_AUTOSETTINGLIST' ), 'generic.png' );
		if($this->user->authorise('core.edit', 'com_chessvn')) JToolBarHelper::editList('autosetting.edit');
		if($this->user->authorise('core.create', 'com_chessvn')) JToolBarHelper::addNew('autosetting.add');
		if($this->user->authorise('core.delete', 'com_chessvn')) JToolBarHelper::deleteList('', 'autosettinglist.delete');
		
		if( (isset($this->rows[0]->published)) && ($this->user->authorise('core.edit', 'com_chessvn')) ){
			JToolBarHelper::divider();
			JToolBarHelper::publish('autosettinglist.publish');
			JToolBarHelper::unpublish('autosettinglist.unpublish');
		}
		...
		//$default_order_field = 'autosettingid';
	...
	protected function auth($area){
		//echo '<pre>' . print_r(JAccess::getActions('com_chessvn','autosetting'),true).'</pre>';die();
		$aclLocal = array();
		foreach(JAccess::getActions('com_chessvn', 'autosetting') as $ar) $aclLocal[] = $ar->name;
		if(in_array($area, $aclLocal)){
			return $this->user->authorise($area, 'com_chessvn', 'autosetting');
		}else{
			$aclGlobal = array();
