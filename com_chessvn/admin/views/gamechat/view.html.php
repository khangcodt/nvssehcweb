 ...
 * @subpackage 	Chessvn
 */
class ChessvnViewGamechat extends JViewLegacy
{
	/**
	...
	function display($tpl = null){
		...
		$isNew = ($this->item->gamechatid == null);

		$text = $isNew ? JText::_( 'JACTION_CREATE' ) : JText::_( 'JACTION_EDIT' );
		JToolBarHelper::title(   JText::_( 'COM_CHESSVN_MENU_GAMECHATLIST' ).': <span>[ ' . $text.' ]</span>' );
		
		if ($isNew)  {
			if($user->authorise('core.create', 'com_chessvn')){
				JToolBarHelper::apply('gamechat.apply');
				JToolBarHelper::save('gamechat.save');
			}
			JToolBarHelper::cancel('gamechat.cancel');
		} else {
			...
			JToolBarHelper::cancel( 'gamechat.cancel', 'JTOOLBAR_CLOSE' );
		}
