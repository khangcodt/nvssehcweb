 ...
 * @subpackage 	Chessvn
 */
class ChessvnModelRating extends JModelAdmin{
	/**
	...
	protected function allowEdit($data = array(), $key = 'id')
	{
		...
		return JFactory::getUser()->authorise('core.edit', 'com_chessvn.rating.'.((int) isset($data[$key]) ? $data[$key] : 0)) or parent::allowEdit($data, $key);
	}
    ...
    protected function prepareTable($table){
        ...
        $nullable ='ratingtype,selfrating';
		if(!$nullable) return;
	 ...
	 * @since	1.6
	 */
	public function getTable($type = 'Rating', $prefix = 'ChessvnTable', $config = array()) 
	{
	...
	{
		// Get the form.
		$form = $this->loadForm('com_chessvn.rating', 'rating', array('control' => 'jform', 'load_data' => $loadData));
		if (empty($form)) 
		{
			return false;
		}
	...
	{
		return 'administrator/components/com_chessvn/models/forms/rating.js';
	}
	...
	protected function loadFormData() 
	{
		...
		$data = JFactory::getApplication()->getUserState('com_chessvn.edit.rating.data', array());
		if (empty($data)) 
		{
			$data = $this->getItem();
		}
