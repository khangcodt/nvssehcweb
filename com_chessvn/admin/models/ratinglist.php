 ...
 * @subpackage 	Chessvn
 */
class ChessvnModelRatinglist extends JModelList{
	...
	protected function populateState($ordering = null, $direction = null){
		...
		$filter_order = $this->getUserStateFromRequest($this->context . '.filter.ordering', 'filter_order', 'ratingid');
	...
	private function _buildQueryOrderBy() {
	    ...
	    $allowedOrders = explode(',', 'ratingid,playerid,chesstype,ratingtype,ratingpoint,selfrating'); 
	...
	private function _buildQueryWhere() {
	    ...
	    if ($search){
			...
			$allowedSearch = explode(',', 'ratingtype');
	...
	protected function getListQuery(){
		//$db = JFactory::getDBO();
		$query = 'SELECT `t1`.*  FROM `#__rating` `t1`  ' . $this->_buildQueryWhere() . $this->_buildQueryOrderBy();
		return $query;
	}
	...
	public function getGenericFieldName($fieldName){
		$db = JFactory::getDBO();
		$db->setQuery( 'SELECT ratingid AS value `$fieldName` AS text FROM `#__rating` ORDER BY `$fieldName`');
		$options = array();
