 ...
 * @subpackage 	Chessvn
 */
class ChessvnModelGamechatlist extends JModelList{
	...
	protected function populateState($ordering = null, $direction = null){
		...
		$filter_order = $this->getUserStateFromRequest($this->context . '.filter.ordering', 'filter_order', 'gamechatid');
	...
	private function _buildQueryOrderBy() {
	    ...
	    $allowedOrders = explode(',', 'gamechatid,gameid,playerid,chatmsg,createddate'); 
	...
	protected function getListQuery(){
		//$db = JFactory::getDBO();
		$query = 'SELECT `t1`.*  FROM `#__gamechat` `t1`  ' . $this->_buildQueryWhere() . $this->_buildQueryOrderBy();
		return $query;
	}
	...
	public function getGenericFieldName($fieldName){
		$db = JFactory::getDBO();
		$db->setQuery( 'SELECT gamechatid AS value `$fieldName` AS text FROM `#__gamechat` ORDER BY `$fieldName`');
		$options = array();
