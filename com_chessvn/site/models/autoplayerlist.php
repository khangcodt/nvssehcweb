 ...
 * @subpackage 	Chessvn
 */
class ChessvnModelAutoplayerlist extends JModelLegacy{
	...
	*/
	...
	public function getData(){
		...
		if (empty( $this->_data )){
			...
			$db = JFactory::getDBO();
			//$query = 'SELECT * FROM `#__autoplayer` WHERE ' . (isset($recordSet->published)?'`published`':'1') . ' = 1 ORDER BY `autoplayerid` ';
			$query = 'SELECT * FROM `#__autoplayer` WHERE 1 ORDER BY `autoplayerid` ';
	...
	public function getTotal(){
		...
		$db->setQuery( 'SELECT COUNT(*) FROM `#__autoplayer` WHERE 1' );
