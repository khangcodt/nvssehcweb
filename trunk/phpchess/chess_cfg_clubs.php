<?php
  ...
  $Contentpage = "cell_chess_cfg_clubs.php";  
  ...
  $cmdCreateClub = $_POST['cmdCreateClub'];
  $txtclubname = $_POST['txtclubname'];

  $bclubcreateerror = false;
  $bclubcreated = false;

  if($cmdCreateClub != "" && $txtclubname != ""){

    if(!$oR3DCQuery->CheckChessClubExists($txtclubname)){

      $oR3DCQuery->CreateChessClub($txtclubname, $_SESSION['id']);
      $bclubcreated = true;

    }else{
      $bclubcreateerror = true;
    }

  }

  $lstclublist = $_POST['lstclublist'];
  $cmdJoinClub = $_POST['cmdJoinClub'];
  
  if($cmdJoinClub != "" && $lstclublist != ""){
    $oR3DCQuery->JoinChessClub($lstclublist, $_SESSION['id']);
  }

  $cmdLeaveClub = $_POST['cmdLeaveClub'];

  if($cmdLeaveClub != ""){
    $oR3DCQuery->LeaveClub($_SESSION['id']);
  }

  $cmdDisband = $_POST['cmdDisband'];

  if($cmdDisband != ""){
    $oR3DCQuery->DisbandClub($_SESSION['id']);
  }
...
<html>
<head>
<title><?php echo GetStringFromStringTable("IDS_PAGETITLES_28", $config);?></title>
