<?php
  ...
  $Contentpage = "cell_chess_buy_credits.php";  
  ...
  $Credits = $_POST['txtcredit'];
  $ExchangeRate = $_POST['txtexchangerate'];
  $TotalAmount = 0.00;
  $CurrencyType = $_POST['txtppcurrency'];
  $cmdPurchase = $_POST['cmdPurchase'];

  $paymentHTML = false;
  $CreditRID = 0;

  if($PID != "" && $UID != "" && $Email != "" && is_numeric($Credits) && is_numeric($ExchangeRate) && $CurrencyType != "" && $cmdPurchase != ""){

    if(strrpos($Credits, ".") === false){

      $TotalAmount = $ExchangeRate * $Credits;

      $CreditRID = $oR3DCQuery->PurchaseCredits($PID, $UID, $Email, $Credits, number_format($ExchangeRate, 2, '.', ''), number_format($TotalAmount, 2, '.', ''));
...
<html>
<head>
<title><?php echo GetStringFromStringTable("IDS_PAGETITLES_59", $config);?></title>
