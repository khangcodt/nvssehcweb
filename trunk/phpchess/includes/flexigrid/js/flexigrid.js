	...
	$.addFlex = function (t, p) {
		...
		p = $.extend({ //apply default properties
            ...
            __mw: { //extendable middleware function holding object
                datacol: function(p, col, val, id) { //middleware for formatting data columns
                    var _col = (typeof p.datacol[col] == 'function') ? p.datacol[col](val, col) : val; //format column using function
                    if(typeof p.datacol['*'] == 'function') { //if wildcard function exists
                        return p.datacol['*'](_col, col, id); //run wildcard function
                    } else {
						...
						$('thead tr:first th', g.hDiv).each( //add cell
							function () {
								...
								if (typeof row.cell == 'undefined') {
                                    ...
                                    td.innerHTML = p.__mw.datacol(p, $(this).attr('abbr'), iHTML, row[p.idProperty]); //use middleware datacol to format cols
								}
