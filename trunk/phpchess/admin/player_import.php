<?php
  ...
  $Contentpage = "cell_admin_player_importer.php";  
  ...
  $txtPlayers = trim($_POST['txtPlayers']);
  $cmdImport = $_POST['cmdImport'];

  $addError = "";

  if($cmdImport != "" && $txtPlayers != ""){

    $strDecoded = base64_decode($txtPlayers);
    $strText = substr($strDecoded, 0, -1);
    $aUserData = explode(",", $strText);

    $i=0;
    $nCount = count($aUserData);

    While($i < $nCount){

      $aUser = preg_split("/\|/", $aUserData[$i]);
      $bNameExists = $oR3DCQuery->UserNameExists($aUser[0]);

      // check if user exists
      if($bNameExists == false){

        // Then add it if it does not
        $oR3DCQuery->InsertTournamentPlayer($aUser[0], $aUser[1], $aUser[2]);

      }else{
        $addError = $addError."".$aUser[0]."<br>";
      }
