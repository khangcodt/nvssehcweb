<?php
  ...
  $Contentpage = "cell_admin_cfg_player_credits.php";  
  ...
  // Variables
  $cmdUpdate = $_POST['cmdUpdate'];
  $txtRate = $_POST['txtRate'];
  $txtCredit = $_POST['txtCredit'];

  if($cmdUpdate != "" && is_numeric($txtCredit) && is_numeric($txtRate)){

    if(strpos($txtCredit, '.') === false){

      $oR3DCQuery->UpdatePlayerCreditConfigInfo($txtCredit, number_format($txtRate,2,'.', ''));

    }

  }

  $oR3DCQuery->GetPlayerCreditConfigInfo($Credits, $ExchangeRate);
?>

<html>
<head>
<title><?php echo GetStringFromStringTable("IDS_PAGETITLES_60", $config);?></title>
