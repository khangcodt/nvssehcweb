<?php
  ...
  $Contentpage = "cell_admin_cfg_srv_filter.php";  
  ...
  require($Root_Path."bin/CLanguage.php");
  ...
  $txtWord = trim($_POST['txtWord']);
  $cmdAddWord = $_POST['cmdAddWord'];
  $lstwordlist = $_POST['lstwordlist'];
  $cmdRemoveWord = $_POST['cmdRemoveWord'];

  if($cmdAddWord != "" && $txtWord != ""){
    $oR3DCQuery->AddWordsToFilter($txtWord);
  }

  if($cmdRemoveWord != "" && $lstwordlist != ""){
    $oR3DCQuery->RemoveFilteredWords($lstwordlist);
  }
...
<html>
<head>
<title><?php echo GetStringFromStringTable("IDS_PAGETITLES_31", $config);?></title>
