<?php
  ...
  $Contentpage = "cell_msg_center_saved.php";  
  ...
  $cmdKeep = trim($_POST['cmdKeep']);
  $txtSavedID = trim($_POST['txtSavedID']);
  ...
  if($cmdDeleteAll != "" && $achkID != ""){
    ...
    while($i < $ncount){
      $oR3DCQuery->DeleteSavedMessage($config, $achkID[$i]);
      $i++;
    }
  ...
  // Delete Message
  if($cmdDelete != "" && $txtSavedID != ""){
    $oR3DCQuery->DeleteSavedMessage($config, $txtSavedID);
  }
