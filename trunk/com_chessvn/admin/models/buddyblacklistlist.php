 ...
 * @subpackage 	Chessvn
 */
class ChessvnModelBuddyblacklistlist extends JModelList{
	...
	protected function populateState($ordering = null, $direction = null){
		...
		$filter_order = $this->getUserStateFromRequest($this->context . '.filter.ordering', 'filter_order', 'bblid');
	...
	private function _buildQueryOrderBy() {
	    ...
	    $allowedOrders = explode(',', 'bblid,playerid,buddyid,blackid,chesstype,reqstate,createdtime'); 
	...
	private function _buildQueryWhere() {
	    ...
	    if ($search){
			...
			$allowedSearch = explode(',', 'reqstate');
	...
	protected function getListQuery(){
		//$db = JFactory::getDBO();
		$query = 'SELECT `t1`.*  FROM `#__buddyblacklist` `t1`  ' . $this->_buildQueryWhere() . $this->_buildQueryOrderBy();
		return $query;
	}
	...
	public function getGenericFieldName($fieldName){
		$db = JFactory::getDBO();
		$db->setQuery( 'SELECT bblid AS value `$fieldName` AS text FROM `#__buddyblacklist` ORDER BY `$fieldName`');
		$options = array();
