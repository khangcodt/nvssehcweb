 ...
 * @subpackage 	Chessvn
 */
class ChessvnModelAutosetting extends JModelAdmin{
	/**
	...
	protected function allowEdit($data = array(), $key = 'id')
	{
		...
		return JFactory::getUser()->authorise('core.edit', 'com_chessvn.autosetting.'.((int) isset($data[$key]) ? $data[$key] : 0)) or parent::allowEdit($data, $key);
	}
    ...
    protected function prepareTable($table){
        ...
        $nullable ='livetimestart,livetimeend';
		if(!$nullable) return;
	 ...
	 * @since	1.6
	 */
	public function getTable($type = 'Autosetting', $prefix = 'ChessvnTable', $config = array()) 
	{
	...
	{
		// Get the form.
		$form = $this->loadForm('com_chessvn.autosetting', 'autosetting', array('control' => 'jform', 'load_data' => $loadData));
		if (empty($form)) 
		{
			return false;
		}
	...
	{
		return 'administrator/components/com_chessvn/models/forms/autosetting.js';
	}
	...
	protected function loadFormData() 
	{
		...
		$data = JFactory::getApplication()->getUserState('com_chessvn.edit.autosetting.data', array());
		if (empty($data)) 
		{
			$data = $this->getItem();
		}
