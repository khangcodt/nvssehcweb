 ...
 * @subpackage 	Chessvn
 */
class ChessvnModelGameoptionlist extends JModelList{
	...
	protected function populateState($ordering = null, $direction = null){
		...
		$filter_order = $this->getUserStateFromRequest($this->context . '.filter.ordering', 'filter_order', 'gameoptionid');
	...
	private function _buildQueryOrderBy() {
	    ...
	    $allowedOrders = explode(',', 'gameoptionid,gameid,gametitle,initiator,wplayerid,bplayerid,chesstype,gamecoin,elowhite,eloblack,elomin,elomax,israting,timemode,maintime,incrementtime,starttime,endtime,createdtime'); 
	...
	private function _buildQueryWhere() {
	    ...
	    if ($search){
			...
			$allowedSearch = explode(',', 'gameid,gametitle,timemode');
	...
	protected function getListQuery(){
		//$db = JFactory::getDBO();
		$query = 'SELECT `t1`.*  FROM `#__gameoption` `t1`  ' . $this->_buildQueryWhere() . $this->_buildQueryOrderBy();
		return $query;
	}
	...
	public function getGenericFieldName($fieldName){
		$db = JFactory::getDBO();
		$db->setQuery( 'SELECT gameoptionid AS value `$fieldName` AS text FROM `#__gameoption` ORDER BY `$fieldName`');
		$options = array();
