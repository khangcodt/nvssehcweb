 ...
 * @subpackage 	Chessvn
 */
class ChessvnModelAutoplayerlist extends JModelList{
	...
	protected function populateState($ordering = null, $direction = null){
		...
		$filter_order = $this->getUserStateFromRequest($this->context . '.filter.ordering', 'filter_order', 'autoplayerid');
	...
	private function _buildQueryOrderBy() {
	    ...
	    $allowedOrders = explode(',', 'autoplayerid,playerid,isonline,isactive,isplayopengame,acceptstart,acceptend,elostart,eloend,resttimestart,resttimeend,chesstype'); 
	...
	protected function getListQuery(){
		//$db = JFactory::getDBO();
		$query = 'SELECT `t1`.*  FROM `#__autoplayer` `t1`  ' . $this->_buildQueryWhere() . $this->_buildQueryOrderBy();
		return $query;
	}
	...
	public function getGenericFieldName($fieldName){
		$db = JFactory::getDBO();
		$db->setQuery( 'SELECT autoplayerid AS value `$fieldName` AS text FROM `#__autoplayer` ORDER BY `$fieldName`');
		$options = array();
