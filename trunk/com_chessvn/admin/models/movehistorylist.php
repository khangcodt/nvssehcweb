 ...
 * @subpackage 	Chessvn
 */
class ChessvnModelMovehistorylist extends JModelList{
	...
	protected function populateState($ordering = null, $direction = null){
		...
		$filter_order = $this->getUserStateFromRequest($this->context . '.filter.ordering', 'filter_order', 'moveid');
	...
	private function _buildQueryOrderBy() {
	    ...
	    $allowedOrders = explode(',', 'moveid,movetime,playerid,move,gameid'); 
	...
	private function _buildQueryWhere() {
	    ...
	    if ($search){
			...
			$allowedSearch = explode(',', 'move,gameid');
	...
	protected function getListQuery(){
		//$db = JFactory::getDBO();
		$query = 'SELECT `t1`.*  FROM `#__movehistory` `t1`  ' . $this->_buildQueryWhere() . $this->_buildQueryOrderBy();
		return $query;
	}
	...
	public function getGenericFieldName($fieldName){
		$db = JFactory::getDBO();
		$db->setQuery( 'SELECT moveid AS value `$fieldName` AS text FROM `#__movehistory` ORDER BY `$fieldName`');
		$options = array();
