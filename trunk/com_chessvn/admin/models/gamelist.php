 ...
 * @subpackage 	Chessvn
 */
class ChessvnModelGamelist extends JModelList{
	...
	protected function populateState($ordering = null, $direction = null){
		...
		$filter_order = $this->getUserStateFromRequest($this->context . '.filter.ordering', 'filter_order', 'gameid');
	...
	private function _buildQueryOrderBy() {
	    ...
	    $allowedOrders = explode(',', 'gameid,status,completionstatus,nextmove,iscastlewl,iscastlews,iscastlebl,iscastlebs,drawrequest,wtimeused,btimeused'); 
	...
	protected function getListQuery(){
		//$db = JFactory::getDBO();
		$query = 'SELECT `t1`.*  FROM `#__game` `t1`  ' . $this->_buildQueryWhere() . $this->_buildQueryOrderBy();
		return $query;
	}
	...
	public function getGenericFieldName($fieldName){
		$db = JFactory::getDBO();
		$db->setQuery( 'SELECT gameid AS value `$fieldName` AS text FROM `#__game` ORDER BY `$fieldName`');
		$options = array();
