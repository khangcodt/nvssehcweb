 ...
 * @subpackage 	Chessvn
 */
class ChessvnModelPlayer extends JModelAdmin{
	/**
	...
	protected function allowEdit($data = array(), $key = 'id')
	{
		...
		return JFactory::getUser()->authorise('core.edit', 'com_chessvn.player.'.((int) isset($data[$key]) ? $data[$key] : 0)) or parent::allowEdit($data, $key);
	}
    ...
    protected function prepareTable($table){
        ...
        $nullable ='chesstitle,coin,favplayer,avatar';
		if(!$nullable) return;
	 ...
	 * @since	1.6
	 */
	public function getTable($type = 'Player', $prefix = 'ChessvnTable', $config = array()) 
	{
	...
	{
		// Get the form.
		$form = $this->loadForm('com_chessvn.player', 'player', array('control' => 'jform', 'load_data' => $loadData));
		if (empty($form)) 
		{
			return false;
		}
	...
	{
		return 'administrator/components/com_chessvn/models/forms/player.js';
	}
	...
	protected function loadFormData() 
	{
		...
		$data = JFactory::getApplication()->getUserState('com_chessvn.edit.player.data', array());
		if (empty($data)) 
		{
			$data = $this->getItem();
		}
