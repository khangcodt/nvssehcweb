 ...
 * @subpackage 	Chessvn
 */
class ChessvnModelTrophy extends JModelAdmin{
	/**
	...
	protected function allowEdit($data = array(), $key = 'id')
	{
		...
		return JFactory::getUser()->authorise('core.edit', 'com_chessvn.trophy.'.((int) isset($data[$key]) ? $data[$key] : 0)) or parent::allowEdit($data, $key);
	}
    ...
    protected function prepareTable($table){
        ...
        $nullable ='trophytime,name,trophytype,imageurl';
		if(!$nullable) return;
	 ...
	 * @since	1.6
	 */
	public function getTable($type = 'Trophy', $prefix = 'ChessvnTable', $config = array()) 
	{
	...
	{
		// Get the form.
		$form = $this->loadForm('com_chessvn.trophy', 'trophy', array('control' => 'jform', 'load_data' => $loadData));
		if (empty($form)) 
		{
			return false;
		}
	...
	{
		return 'administrator/components/com_chessvn/models/forms/trophy.js';
	}
	...
	protected function loadFormData() 
	{
		...
		$data = JFactory::getApplication()->getUserState('com_chessvn.edit.trophy.data', array());
		if (empty($data)) 
		{
			$data = $this->getItem();
		}
