 ...
 * @subpackage 	Chessvn
 */
class ChessvnModelPlayerlist extends JModelList{
	...
	protected function populateState($ordering = null, $direction = null){
		...
		$filter_order = $this->getUserStateFromRequest($this->context . '.filter.ordering', 'filter_order', 'playerid');
	...
	private function _buildQueryOrderBy() {
	    ...
	    $allowedOrders = explode(',', 'playerid,userid,chesstitle,coin,favplayer,avatar'); 
	...
	private function _buildQueryWhere() {
	    ...
	    if ($search){
			...
			$allowedSearch = explode(',', 'chesstitle,favplayer,avatar');
	...
	protected function getListQuery(){
		//$db = JFactory::getDBO();
		$query = 'SELECT `t1`.*  FROM `#__player` `t1`  ' . $this->_buildQueryWhere() . $this->_buildQueryOrderBy();
		return $query;
	}
	...
	public function getGenericFieldName($fieldName){
		$db = JFactory::getDBO();
		$db->setQuery( 'SELECT playerid AS value `$fieldName` AS text FROM `#__player` ORDER BY `$fieldName`');
		$options = array();
