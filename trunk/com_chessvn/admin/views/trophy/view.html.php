 ...
 * @subpackage 	Chessvn
 */
class ChessvnViewTrophy extends JViewLegacy
{
	/**
	...
	function display($tpl = null){
		...
		$isNew = ($this->item->trophyid == null);

		$text = $isNew ? JText::_( 'JACTION_CREATE' ) : JText::_( 'JACTION_EDIT' );
		JToolBarHelper::title(   JText::_( 'COM_CHESSVN_MENU_TROPHYLIST' ).': <span>[ ' . $text.' ]</span>' );
		
		if ($isNew)  {
			if($user->authorise('core.create', 'com_chessvn')){
				JToolBarHelper::apply('trophy.apply');
				JToolBarHelper::save('trophy.save');
			}
			JToolBarHelper::cancel('trophy.cancel');
		} else {
			...
			JToolBarHelper::cancel( 'trophy.cancel', 'JTOOLBAR_CLOSE' );
		}
