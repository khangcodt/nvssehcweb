...
<?php
	...
	$this->lists['order'] = $this->escape($this->state->get('filter.ordering', 'gameoptionid'));// default order is primary key
...
<?php endif; ?>
	...
	<th class="jcb_fieldDiv jcb_fieldLabel"><?php $numCols++; ?>
		<?php echo JHTML::_('grid.sort', JText::_('COM_CHESSVN_GAMEOPTION_GAMEOPTIONID_LABEL'), 'gameoptionid', $this->lists['order_Dir'], $this->lists['order']); ?>
	</th>
			...
			<th width="20" class="jcb_fieldDiv jcb_fieldLabel nowrap"><?php $numCols++; ?>
				...
				<?php if ($this->auth('core.edit') && ($this->lists['order']=='ordering')): ?>
					<?php echo JHtml::_('grid.order',  $this->rows, 'filesave.png', 'gameoptionlist.saveorder'); ?>
				<?php endif;?>				
			</th>
			<?php endif; ?>
	...
	for ($i=0, $n=count( $this->rows ); $i < $n; $i++)	{
		$row = &$this->rows[$i];
		$checked = JHTML::_('grid.id', $i, $row->gameoptionid);
		$link = JRoute::_( 'index.php?option=com_chessvn&task=gameoption.edit&gameoptionid='. $row->gameoptionid);
		$k = 1 - $k;
		?>
		<tr class="<?php echo "row$k"; ?>">
			...
			<td>
				<?php if (isset($row->checked_out) && ($row->checked_out)) : ?>
					<?php echo JHtml::_('jgrid.checkedout', $i, 'someone...', $row->checked_out_time, 'gameoptionlist.', $this->auth('gameoption.checkin')); ?>
				<?php endif; ?>				
	...
	<td class="jcb_fieldDiv jcb_fieldValue">
		<?php echo $row->gameoptionid; ?>
			...
			<td class="order nowrap">
				...
				?>
					<span><?php echo $page->orderUpIcon( $i, $i>0, 'gameoptionlist.'.$up, JText::_( 'JLIB_HTML_MOVE_UP' ), $enable ); ?></span>
					<span><?php echo $page->orderDownIcon( $i, $n, $i<$n, 'gameoptionlist.'.$down, JText::_( 'JLIB_HTML_MOVE_DOWN' ), $enable ); ?></span>					
			...
			<td>
				<?php if($this->j3x): ?>
					<?php echo JHtml::_('jgrid.published', $row->published, $i, 'gameoptionlist.', true); ?>
				<?php else: ?>
					<?php echo JHTML::_('grid.published', $row, $i, 'tick.png', 'publish_x.png', 'gameoptionlist.'); ?>
				<?php endif; ?>
			</td>
			<?php endif; ?>
		</tr>
...
<input type="hidden" name="view" value="gameoptionlist" />
