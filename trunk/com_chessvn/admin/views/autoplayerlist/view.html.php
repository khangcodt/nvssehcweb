 ...
 * @subpackage 	Chessvn
 */
class ChessvnViewAutoplayerlist extends JViewLegacy
{
	/**
	 * Autoplayerlist view display method
	 * @return void
	 **/
	function display($tpl = null){
		...
		JToolBarHelper::title( JText::_( 'COM_CHESSVN_MENU_AUTOPLAYERLIST' ), 'generic.png' );
		if($this->user->authorise('core.edit', 'com_chessvn')) JToolBarHelper::editList('autoplayer.edit');
		if($this->user->authorise('core.create', 'com_chessvn')) JToolBarHelper::addNew('autoplayer.add');
		if($this->user->authorise('core.delete', 'com_chessvn')) JToolBarHelper::deleteList('', 'autoplayerlist.delete');
		
		if( (isset($this->rows[0]->published)) && ($this->user->authorise('core.edit', 'com_chessvn')) ){
			JToolBarHelper::divider();
			JToolBarHelper::publish('autoplayerlist.publish');
			JToolBarHelper::unpublish('autoplayerlist.unpublish');
		}
		...
		//$default_order_field = 'autoplayerid';
	...
	protected function auth($area){
		//echo '<pre>' . print_r(JAccess::getActions('com_chessvn','autoplayer'),true).'</pre>';die();
		$aclLocal = array();
		foreach(JAccess::getActions('com_chessvn', 'autoplayer') as $ar) $aclLocal[] = $ar->name;
		if(in_array($area, $aclLocal)){
			return $this->user->authorise($area, 'com_chessvn', 'autoplayer');
		}else{
			$aclGlobal = array();
