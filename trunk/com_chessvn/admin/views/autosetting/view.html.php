 ...
 * @subpackage 	Chessvn
 */
class ChessvnViewAutosetting extends JViewLegacy
{
	/**
	...
	function display($tpl = null){
		...
		$isNew = ($this->item->autosettingid == null);

		$text = $isNew ? JText::_( 'JACTION_CREATE' ) : JText::_( 'JACTION_EDIT' );
		JToolBarHelper::title(   JText::_( 'COM_CHESSVN_MENU_AUTOSETTINGLIST' ).': <span>[ ' . $text.' ]</span>' );
		
		if ($isNew)  {
			if($user->authorise('core.create', 'com_chessvn')){
				JToolBarHelper::apply('autosetting.apply');
				JToolBarHelper::save('autosetting.save');
			}
			JToolBarHelper::cancel('autosetting.cancel');
		} else {
			...
			JToolBarHelper::cancel( 'autosetting.cancel', 'JTOOLBAR_CLOSE' );
		}
