 ...
 * @subpackage 	Chessvn
 */
class ChessvnViewGameoption extends JViewLegacy
{
	/**
	...
	function display($tpl = null){
		...
		$isNew = ($this->item->gameoptionid == null);

		$text = $isNew ? JText::_( 'JACTION_CREATE' ) : JText::_( 'JACTION_EDIT' );
		JToolBarHelper::title(   JText::_( 'COM_CHESSVN_MENU_GAMEOPTIONLIST' ).': <span>[ ' . $text.' ]</span>' );
		
		if ($isNew)  {
			if($user->authorise('core.create', 'com_chessvn')){
				JToolBarHelper::apply('gameoption.apply');
				JToolBarHelper::save('gameoption.save');
			}
			JToolBarHelper::cancel('gameoption.cancel');
		} else {
			...
			JToolBarHelper::cancel( 'gameoption.cancel', 'JTOOLBAR_CLOSE' );
		}
