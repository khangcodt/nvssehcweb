 ...
 * @subpackage 	Chessvn
 */
class ChessvnViewAutoplayer extends JViewLegacy
{
	/**
	...
	function display($tpl = null){
		...
		$isNew = ($this->item->autoplayerid == null);

		$text = $isNew ? JText::_( 'JACTION_CREATE' ) : JText::_( 'JACTION_EDIT' );
		JToolBarHelper::title(   JText::_( 'COM_CHESSVN_MENU_AUTOPLAYERLIST' ).': <span>[ ' . $text.' ]</span>' );
		
		if ($isNew)  {
			if($user->authorise('core.create', 'com_chessvn')){
				JToolBarHelper::apply('autoplayer.apply');
				JToolBarHelper::save('autoplayer.save');
			}
			JToolBarHelper::cancel('autoplayer.cancel');
		} else {
			...
			JToolBarHelper::cancel( 'autoplayer.cancel', 'JTOOLBAR_CLOSE' );
		}
