...
<div id="mod-chat">
    ...
    <div id="enter-message">
        ...
        <a onclick="sendMessage()" id="send-message">Send</a>
    </div>
</div>
<script type="text/javascript">
    ...
    jQuery(document).ready(function(){
        if(timeupdate == 10000){
            jQuery('#text-message').attr('disabled','disabled');
            jQuery('#send-message').css('pointer-events', 'none');
        }else{
            jQuery('#text-message').removeAttr('disabled');
            jQuery('#send-message').css('pointer-events', '');
        }
        getXmlChat();
